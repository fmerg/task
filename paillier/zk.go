...
// Generate ZK proof of knoledge that the provided cipher is the encyption of
// x under the provided public Paillier key. r should be the randomness used
// for encryption
...
// Verify the present ZK-proof against cipher w and receiver's Paillier key
func (proof *ZKProof) Verify(w *big.Int, key *Key) (bool, error) {
  ...
  lu2 := new(big.Int).Exp(key.Gamma, s1, key.NTo2)
  lu2.Mul(lu2, new(big.Int).Exp(s2, key.N, key.NTo2)).Mod(lu2, key.NTo2)
  ru2 := new(big.Int).Mul(u2, new(big.Int).Exp(w, e, key.NTo2))
  ru2.Mod(ru2, key.NTo2)
